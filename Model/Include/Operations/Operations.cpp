...
Operations::Operations() : SC(Scope::getScope())
{
    castTable = {
        {"int", &Operations::intCast},
        {"signed int", &Operations::intCast},
        {"long", &Operations::intCast},
        {"long int",  &Operations::intCast},
        {"signed long int",  &Operations::intCast},

        {"char", &Operations::charCast},
        {"float", &Operations::floatCast},
        {"double", &Operations::doubleCast},
        {"bool", &Operations::boolCast},
        {"char", &Operations::charCast},
        {"short", &Operations::shortCast}

    };
}

Operations::Operations(std::string expression1) : SC(Scope::getScope()), expression(expression1)
{
    castTable = {
        {"int", &Operations::intCast},
        {"signed int", &Operations::intCast},
    ...
    //std::cout <<  "llllllll = " << expression1 << std::endl;
    //Operations();
    result = resolveOperation(expression);

}

std::string Operations::plusOne(std::string num)
{
...
std::string Operations::char_cast_to_int(std::string str)
{
    if (str[0] == '\'') {
        if (str[1] == '\\') {
            if (str[2] == 'a') {
...
std::string Operations::intCast(std::string num)
{
    int integer_value = 0;
...
std::string Operations::charCast(std::string num)
{
    char char_value = 0;
    std::istringstream iss(num);
    long double number;
...
std::string Operations::floatCast(std::string num)
{
    return num;
}

std::string Operations::doubleCast(std::string val)
{
    std::string ret_val = "";
    ...
    bool neg = false;
    if(val == "") //double x{}  val = "" or double d = d
    {
        return "0";
    }
...
std::string Operations::boolCast(std::string num)
{
    bool result = 0;
    std::istringstream iss(num);
    iss >> result;
...
std::string Operations::shortCast(std::string num)
{
    short short_value = 0;
    bool isNegative = false;
    if (num == "inf" ){
...
std::string Operations::logicalNot(std::string var_name)
{
    //logica operations doesn't work with floating_point numbers
    std::string var_type = SC -> getValueByName(var_name).second;
...
std::string Operations::bitwiseNot(std::string var_name)
{
    //double and float numbers aren't considered
    std::string var_type = SC -> getValueByName(var_name).second;
    std::string value = SC -> getValueByName(var_name).first;
    int result;
    if (var_type.find("unsigned") != std::string::npos) {
        // must support also unsigned versions, it will have another implementation
    } else {
        if(var_type.find("char") != std::string::npos) {
            std::vector<std::string> tokens = tokenizing(var_type);
            result = std::stoll(tokens[tokens.size() - 1]);
        } else {
            if (std::isdigit(var_name[0])) {
               result = std::stoi(var_name);
            } else {
...
std::string Operations::prefixIncrement(std::string var_name)
{
    //std::cout << "prefix incr1 : " << var_name << std::endl;
    bool type_contains_char = false;
    std::string var_type = SC -> getValueByName(var_name).second;
    //std::cout << "prefix incr2 : " << var_name << std::endl;
    ...
    } else if (var_type.find("float") != std::string::npos || var_type.find("double") != std::string::npos) {
        value = std::to_string(std::stold(SC -> getValueByName(var_name).first));
    } else {
        value  = SC -> getValueByName(var_name).first;
        std::cout << "prefix incr3 : " << var_name << std::endl;
    ...
    value = (this->*Operations::castTable[var_type])(value);

    if (type_contains_char) {
        SC -> changeType(var_name, var_type + " " + tmp_value_for_char);
    }
    ...
    std::cout <<"prefix incr:::: " <<  value << std::endl;
    return value;
}

std::string Operations::prefixDecrement(std::string expr)
{
    return expr;
}

std::string Operations::postfixIncrement(std::string var_name)
{
    bool type_contains_char = false;
    std::string var_type = SC -> getValueByName(var_name).second;
    std::string value;
...
std::string Operations::postfixDecrement(std::string expr)
{
    return expr;
}

std::string Operations::resolveOperation(std::string expr)
{
    //std::cout << "resolveExpr: " << expr << std::endl;
    std::string result;
    ...
    if (operation == "!") {
        ...
        //std::cout << "prefix is working " << std::endl;
        //std::cout << expr.substr(index) << std::endl;
        result = prefixIncrement(expr.substr(index));
    } else if (operation == "--") {
        result = prefixDecrement(expr.substr(index));
    } else if (operation == ""){ // op == ""
        if (expr[expr.size() - 1] == '+') {
            //std::cout << "postfix is working " << std::endl;
    ...
    //std::cout << "resolveExpr1 : " << result << std::endl;

    return result;
}

std::string Operations::callToAst(std::string expr)
{
...
std::string Operations::operator +=(std::string expr)
{
    std::vector<std::string> tokens = tokenizing(expr);
    // x += y
    tokens[1] = " + ";
...
std::pair<std::string, std::string> Operations::getResultAndVariable_type()
{
    return {result, var_type};
}

std::vector<std::string> Operations::tokenizing(std::string str)
{
